...
void IRBuilder::AddCapabilityFor(const DataType& dtype) {
  ...
  if (dtype.is_int() || dtype.is_uint()) {
    ...
    } else if (dtype.lanes() > 4) {
      // NOTE(fucheng): WebGPU does not support this capability.
      //capabilities_used_.insert(spv::CapabilityVector16);
    }
  }
...
Value IRBuilder::Mod(Value a, Value b) {
  DLOG(INFO) << "IRBuilder::Mod, dtype " << a.stype.type;
  ICHECK_EQ(a.stype.id, b.stype.id);
  if (a.stype.type.is_int()) {
