...
runtime::Module BuildSPIRV(IRModule mod, Target target, bool webgpu_restriction) {
  ...
  for (auto kv : mod->functions) {
    ...
    DLOG(INFO) << "Build function " << f_name;
    DLOG(INFO) << f;
    ...
    if (auto path = std::getenv("TVM_VULKAN_DEBUG_SHADER_SAVEPATH")) {
      if (*path) {
        ...
        char time_str_arr[64];
        strftime(time_str_arr, 50, "%Y%m%d%H%M%S", curr_tm);
        std::string time_str(time_str_arr);

        std::ofstream(prefix + "tir_" + time_str + ".txt") << f;
        std::ofstream(prefix + "spv_" + time_str + ".txt") << spirv_tools.BinaryToText(shader.data);
        std::ofstream(prefix + "spv_" + time_str + ".spv", std::ios::binary)
