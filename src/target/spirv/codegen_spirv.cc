...
runtime::VulkanShader CodeGenSPIRV::BuildFunction(const PrimFunc& f, const std::string& name) {
  this->InitFuncState();
  DLOG(INFO) << "Check has zon-zero attr";
  ...
  DLOG(INFO) << "Collect args";
  for (Var arg : f->params) {
    DLOG(INFO) << arg->name_hint;
  ...
  DLOG(INFO) << "Check used shared memory bytes, used " << shared_memory_bytes_used_;
  ...
  DLOG(INFO) << "Check workgroup size, (" << workgroup_size_[0] << ", "
      << workgroup_size_[1] << ", " << workgroup_size_[2] << ")";
  ICHECK_LE(workgroup_size_[0], 256);
  ICHECK_LE(workgroup_size_[1], 256);
  ICHECK_LE(workgroup_size_[2], 64);
...
spirv::Value CodeGenSPIRV::VisitExpr_(const ModNode* op) {
  ...
  return builder_->Sub(a, builder_->Mul(b, builder_->Div(a, b)));
}
...
spirv::Value CodeGenSPIRV::VisitExpr_(const CallNode* op) {
  ...
  if (op->op.same_as(builtin::call_spirv_pure_glsl450())) {
    ...
    if (func_name == "__dp4a") {
      ...
      DLOG(INFO) << "SPIR-V shader cannot make extern calls.  Graph contains extern \""
...
void CodeGenSPIRV::VisitStmt_(const ForNode* op) {
  ...
  ICHECK(op->kind != ForKind::kUnrolled);
