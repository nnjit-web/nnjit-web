...
void CodeGenLLVM::Optimize() {
  ...
  //LOG(INFO) << "builder.OptLevel " << builder.OptLevel;
  unsigned int size_level = 2;
#if TVM_LLVM_VERSION >= 50
  builder.Inliner = llvm::createFunctionInliningPass(builder.OptLevel, size_level, false);
#else
  builder.Inliner = llvm::createFunctionInliningPass(builder.OptLevel, size_level);
#endif
#define TVM_LLVM_LOOP_VECTORIZATION
#ifdef TVM_LLVM_LOOP_VECTORIZATION
  ...
  builder.LoopsInterleaved = false;
#endif
...
void CodeGenLLVM::CreateSerialFor(llvm::Value* begin, llvm::Value* end, llvm::Value* stride,
  ...
  //LOG(INFO) << "CreateSerialFor: loop_var_name " << loop_var_name;
  llvm::LLVMContext* ctx = llvm_target_->GetContext();
