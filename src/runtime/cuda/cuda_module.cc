...
class CUDAModuleNode : public runtime::ModuleNode {
 public:
  ...
  CUfunction GetFunc(int device_id, const std::string& func_name) {
    LOG(INFO) << "GetFunc: device_id " << device_id << ", func_name " << func_name;
    std::lock_guard<std::mutex> lock(mutex_);
    // must recheck under the lock scope
    if (module_[device_id] == nullptr) {
      LOG(INFO) << "cuModuleLoadData: start";
      //LOG(INFO) << "cuModuleLoadData: data " << data_;
      CUDA_DRIVER_CALL(cuModuleLoadData(&(module_[device_id]), data_.c_str()));
      LOG(INFO) << "cuModuleLoadData: done";
    }
    LOG(INFO) << "cuModuleGetFunction: start";
    CUfunction func;
    CUresult result = cuModuleGetFunction(&func, module_[device_id], func_name.c_str());
    LOG(INFO) << "cuModuleGetFunction: done";
    if (result != CUDA_SUCCESS && result != CUDA_ERROR_STUB_LIBRARY) {
      const char* msg;
...
class CUDAWrappedFunc {
 public:
  ...
  void operator()(TVMArgs args, TVMRetValue* rv, void** void_args) const {
    ...
    LOG(INFO) << "cuLaunchKernel: done";
    if (result != CUDA_SUCCESS && result != CUDA_ERROR_DEINITIALIZED && result != CUDA_ERROR_STUB_LIBRARY) {
      const char* msg;
      cuGetErrorName(result, &msg);
...
PackedFunc CUDAModuleNode::GetFunction(const std::string& name,
  ...
  LOG(INFO) << "GetFunction: name " << name;
