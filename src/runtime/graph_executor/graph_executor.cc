...
#if 0
  // Matmul only.
  if (param.func_name.find("dense_pack") == std::string::npos &&
      param.func_name.find("batch_matmul") == std::string::npos) {
    return {[]() {}, arg_ptr};
  }
#endif
...
#if 0
  // conv2d, max_pool2d
  if (param.func_name.find("tvmgen_default_fused_nn_conv2d_4") == std::string::npos) {
    return {[]() {}, arg_ptr};
  }

  //if (param.func_name.find("conv2d") != std::string::npos) {
  //  return {[]() {}, arg_ptr};
  //}
#endif

#if 1
  // NOTE(fucheng): For WebGPU, we need to add kernel functions to import cache.
  for (int i = 0; i < 10; i ++) {
    std::string kernel_name = param.func_name + "_kernel" + std::to_string(i);
    tvm::runtime::PackedFunc pf = module_.GetFunction(kernel_name, true);
    if (pf != nullptr) {
#if 0
      auto it = import_cache_.find(kernel_name);
      if (it == import_cache_.end()) {
        LOG(INFO) << "Add function to import cache: " << kernel_name;
        import_cache_.insert(std::make_pair(
            kernel_name, std::make_shared<tvm::runtime::PackedFunc>(pf)));
      }
#endif
#if 0
      LOG(INFO) << "Register function to global: " << kernel_name;
      TVMFuncRegisterGlobal(kernel_name.c_str(), &pf, 1);
#endif
    }
  }
#endif
