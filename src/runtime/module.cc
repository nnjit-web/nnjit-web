...
const PackedFunc* ModuleNode::GetFuncFromEnv(const std::string& name) {
  //LOG(INFO) << "Module[" << type_key() << "]: GetFuncFromEnv name " << name;
...
#if 0
    const std::string tkey(m->type_key());
    if (tkey == "webgpu") {
      continue;
    }
#endif
...
#if 0
  if (pf == nullptr) {
    pf = this->GetFunction(name, true);
  }
#endif
#if 0
  // NOTE(fucheng): For WebGPU, we need to get function from SystemLib.
  if (pf == nullptr) {
   const PackedFunc* f = Registry::Get("runtime.SystemLib");
   Module m = (*f)();
   pf = m.GetFunction(name, true);
  }
#endif
  if (pf == nullptr) {
    ...
    ICHECK(f != nullptr) << "Cannot find function " << name
                         ...
                         << " library.";
    return f;
    //import_cache_.insert(std::make_pair(name, std::shared_ptr<PackedFunc>(const_cast<PackedFunc*>(f))));
...
bool RuntimeEnabled(const std::string& target) {
  ...
  if (target == "cpu") {
    ...
    return (*pf)(target);
  } else if (target.length() >= 4 && target.substr(0, 4) == "wasm") {
    const PackedFunc* pf = runtime::Registry::Get("codegen.wasm_target_enabled");
    if (pf == nullptr) return false;
    return (*pf)(target);
  } else {
