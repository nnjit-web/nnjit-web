...
class DefaultTimerNode : public TimerNode {
 public:
  ...
  virtual void Stop() {
    ...
    duration_ms_ = stop - start_;
  }
  virtual int64_t SyncAndGetElapsedNanos() { return duration_.count(); }
  virtual double SyncAndGetElapsedMs() { return duration_ms_.count(); }
...
class CPUTimerNode : public TimerNode {
 public:
  virtual void Start() { start_ = NOW(); }
  virtual void Stop() {
    stop_ = NOW();
    duration_ = stop_ - start_;
    duration_ms_ = stop_ - start_;
  }
  virtual int64_t SyncAndGetElapsedNanos() { return duration_.count(); }
  virtual double SyncAndGetElapsedMs() {
  ...
  auto ftimer = [pf, dev, number, repeat, min_repeat_ms, limit_zero_time_iterations,
    ...
    for (int i = 0; i < repeat; ++i) {
      ...
      do {
        ...
        double t_ms = t->SyncAndGetElapsedMs();
        if (t_ms == 0) absolute_zero_times++;
        duration_ms = t_ms;
        //LOG(INFO) << "duration_ms " << duration_ms;
      } while (duration_ms < min_repeat_ms && absolute_zero_times < limit_zero_time_iterations);

      double speed = double(duration_ms) / 1e3 / number;
