...
class GPUCodeVerifier : public StmtExprVisitor {
 public:
  std::vector<String> Verify(Stmt stmt, int64_t max_local_memory_per_block,
    ...
    min_block_x_ = min_block_y_ = min_block_z_ = 1;
    min_thread_x_ = min_thread_y_ = min_thread_z_ = 1;
    ...
    if (op->attr_key == attr::thread_extent || op->attr_key == attr::virtual_thread) {
      ...
      if (name == "threadIdx.x" || name == "threadIdx.y" || name == "threadIdx.z" ||
        ...
        if (!visited_threads_.count(name)) {
          ...
          auto err_gt = [this](std::string id, size_t ext, size_t m) {
            if (ext > m) {
              std::stringstream s;
          ...
          auto err_lt = [this](std::string id, size_t ext, size_t m) {
            if (ext < m) {
              std::stringstream s;
              s << "Extent of " << id << " (" << ext << ") is less than maximum allowed (" << m
                << ");";
              errors_.push_back(s.str());
            }
          };
          ...
          if (name == "threadIdx.x") {
            err_lt("threadIdx.x", length, min_thread_x_);
            err_gt("threadIdx.x", length, max_thread_x_);
            thread_x_extent_ = length;
          } else if (name == "threadIdx.y") {
            err_lt("threadIdx.y", length, min_thread_y_);
            err_gt("threadIdx.y", length, max_thread_y_);
            thread_y_extent_ = length;
          } else if (name == "threadIdx.z") {
            err_lt("threadIdx.z", length, min_thread_z_);
            err_gt("threadIdx.z", length, max_thread_z_);
            thread_z_extent_ = length;
          } else if (name == "vthread") {
            err_gt("vthread", length, max_vthread_);
          }
        } else {
      ...
      } else if (name == "blockIdx.x" || name == "blockIdx.y" || name == "blockIdx.z") {
        size_t length = static_cast<size_t>(extent->value);
        
        auto err = [this](std::string id, size_t ext, size_t m) {
          if (ext < m) {
            std::stringstream s;
            s << "Extent of " << id << " (" << ext << ") is less than minimum allowed (" << m
              << ");";
            errors_.push_back(s.str());
          }
        };
        ...
        if (name == "blockIdx.x") {
          err("blockIdx.x", length, min_block_x_);
        } else if (name == "blockIdx.y") {
          err("blockIdx.y", length, min_block_y_);
        } else if (name == "blockIdx.z") {
          err("blockIdx.z", length, min_block_z_);
        }
      }
 ...
 private:
  ...
  size_t min_block_x_, min_block_y_, min_block_z_;
  size_t min_thread_x_, min_thread_y_, min_thread_z_;
