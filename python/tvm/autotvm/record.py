...
from . import task
from .task import ConfigEntity, ApplyHistoryBest, ApplyHistoryBestByIndex
...
def encode(inp, result, protocol="json"):
    ...
    if protocol == "json":
        save_error_result = get_os_env_var_bool("TVM_ENABLE_SAVE_ERROR_RESULT", True)
        if (not save_error_result) and (result.error_no != 0):
            return None
        json_dict = {
...
def pick_best_config_by_targetkey(in_file, key):
    context = load_from_file(in_file)
    context, _ = itertools.tee(context)
    best_context = ApplyHistoryBest(context)
    #print("recorder.py:", key)
    #print("recorder.py:", best_context.best_by_targetkey)
    if key in best_context.best_by_targetkey:
        inp, _ = best_context.best_by_targetkey[key]
        return inp.config
    else:
        return None


def pick_best_costs_by_targetkey(in_file, idx):
    context = load_from_file(in_file)
    context, _ = itertools.tee(context)
    best_context = ApplyHistoryBestByIndex(context, idx)
    #print("recorder.py:", best_context.best_by_targetkey)
    costs = []
    for key in best_context.best_by_targetkey:
        _, res = best_context.best_by_targetkey[key]
        cost = np.mean(res.costs)
        costs.append(cost)
    return costs, best_context.max_workload_count
