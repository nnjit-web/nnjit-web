...
class ApplyHistoryBest(DispatchContext):
    ...
    def load(self, records: Union[Records, Iterable[Records]]):
        ...
        for inp, res in flattened_records:
            #print("dispatcher.py: inp {}, res {}".format(inp, res))
            counter += 1
            if res.error_no != 0:
                continue
    ...
    def _query_inside(self, target, workload):
        ...
        #print("dispatcher.py: key {}".format(key))
        ...
        for k in target.keys:
            ...
            if key in self.best_by_targetkey:
                ...
                print("dispatcher.py: best_config {}".format(inp.config))
                return inp.config
            
        #print("dispatcher.py: no config found")
...
class ApplyHistoryBestByIndex(DispatchContext):
    def __init__(self, records: Union[None, Records, Iterable[Records]], idx: int):
        super(ApplyHistoryBestByIndex, self).__init__()

        self.index = idx
        self.max_workload_count = 0
        self.best_by_targetkey = {}
        self.best_by_model = {}
        self._best_user_defined = {}
    ...
    def load(self, records: Union[Records, Iterable[Records]]):
        ...
        for inp, res in flattened_records:
            ...
            counter += 1
            workload_counter += 1
            #print("dispatcher.py: counter %d, workload_counter %d" % (counter, workload_counter))
            if res.error_no != 0:
                continue

            if last_workload is None or last_workload != inp.task.workload:
                if workload_counter > self.max_workload_count:
                    self.max_workload_count = workload_counter
                workload_counter = 1
                last_workload = inp.task.workload
            else:
                if (workload_counter - 1) > self.index:
                    continue
            ...
            #print("dispatcher.py: best_by_targetkey", best_by_targetkey)
