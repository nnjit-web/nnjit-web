...
class CodegenRequestHandler(tornado.web.RequestHandler):
    def initialize(self, database):
        self.database = database

    def get(self, *args, **kwargs):
        self.write("Hello, codegen!")

    def post(self, *args, **kwargs):
        import json
        codegen_post = json.loads(self.request.body)
        action = codegen_post["action"]
        proxy_key = codegen_post["key"]
        subprocess_pool = self.database
        if action == "start":
            model_name = codegen_post["model"]
            backend_name = codegen_post["backend"]
            engine_name = codegen_post["engine"]
            logging.info(f"proxy.py: Start codegen: key {proxy_key}, model {model_name}, backend {backend_name}, engine {engine_name}")
            status = self.get_codegen_status(proxy_key)
            if status == CodegenStatus.STOPPED:
                if proxy_key in subprocess_pool:
                    self.kill_model_codegen_subprocess(proxy_key)
                p = self.create_codegen_subprocess(proxy_key, model_name, backend_name, engine_name)
                subprocess_pool[proxy_key] = p
                self.write("Codegen starts")
            elif status == CodegenStatus.PAUSED:
                self.resume_codegen_subprocess(proxy_key)
                self.write("Codegen continues")
            else:
                self.write("Codegen is running")
        elif action == "pause":
            logging.info(f"proxy.py: Pause codegen: key {proxy_key}")
            status = self.get_codegen_status(proxy_key)
            if status == CodegenStatus.RUNNING:
                self.suspend_codegen_subprocess(proxy_key)
                self.write("Codegen pauses")
            else:
                self.write("Codegen is not running")
        elif action == "stop":
            logging.info(f"proxy.py: Stop codegen: key {proxy_key}")
            status = self.get_codegen_status(proxy_key)
            if status != CodegenStatus.STOPPED:
                self.kill_model_codegen_subprocess(proxy_key)
                del subprocess_pool[proxy_key]
                self.write("Codegen stops")
            else:
                self.write("Codegen is stopped")
        elif action == "query":
            '''
            if proxy_key in subprocess_pool:
                proc = subprocess_pool[proxy_key]
                pid = proc.pid
                if psutil.pid_exists(pid):
                    proc = psutil.Process(pid=pid)
                    out = "ppid %d, status %s" % (pid, str(proc.status()))
                    for p in proc.children(recursive=True):
                        out += "\ncpid %d, status %s" % (p.pid, str(p.status()))
                    self.write(out)
                else:
                    self.write("Can not find codegen pid %d" % pid)
            else:
                self.write("Codegen has not been started yet")
            '''
            status = self.get_codegen_status(proxy_key)
    ...
    def resume_codegen_subprocess(self, proxy_key):
        subprocess_pool = self.database
        proc = subprocess_pool[proxy_key]
        pid = proc.pid
        if psutil.pid_exists(pid):
    ...
    def kill_model_codegen_subprocess(self, proxy_key):
        subprocess_pool = self.database
        proc = subprocess_pool[proxy_key]
        #proc.terminate()
        pid = proc.pid
    ...
    def get_codegen_status(self, proxy_key):
        subprocess_pool = self.database
        if proxy_key in subprocess_pool:
            proc = subprocess_pool[proxy_key]
            pid = proc.pid
            if psutil.pid_exists(pid):
                ...
                if proc.status() == psutil.STATUS_STOPPED:
                    return CodegenStatus.PAUSED
                elif proc.status() == psutil.STATUS_ZOMBIE:
                    return CodegenStatus.STOPPED
                elif proc.status() == psutil.STATUS_RUNNING:
                    return CodegenStatus.RUNNING
        ...
        if web_port:
            ...
            if index_page:
                ...
                if certfile is not None and keyfile is not None:
                    logging.info("Serving RPC index html page at https://localhost:%d", web_port)
                else:
            ...
            for fname in resource_files:
                ...
                logging.info(pair)
            self.subprocess_pool = {}
            handlers.append((r"/codegen", CodegenRequestHandler, dict(database=self.subprocess_pool)))
