...
def conv2d_strategy_cuda(attrs, inputs, out_type, target):
    ...
    if groups == 1:
        if layout == "NCHW":
            ...
            '''
            is_webgpu = topi.cuda.cuda_utils.is_webgpu_cuda()
            print("is_webgpu", is_webgpu)
            if is_webgpu and data.dtype in ["float32"]:
                # NOTE(fucheng): quantized compute and schedule is OK for WebGPU.
                compute_conv2d_NHWC_interleaved = topi.arm_cpu.compute_conv2d_NHWC_quantized_interleaved
                schedule_conv2d_NHWC_interleaved = topi.arm_cpu.schedule_conv2d_NHWC_quantized_interleaved
                strategy.add_implementation(
                    wrap_compute_conv2d(compute_conv2d_NHWC_interleaved),
                    wrap_topi_schedule(schedule_conv2d_NHWC_interleaved),
                    name="conv2d_NHWC_interleaved.arm_cpu",
                    plevel=25,
                )
                return strategy
            '''
...
@conv2d_gemm_without_weight_transform_strategy.register(["cuda", "gpu"])
def conv2d_gemm_without_weight_transform_strategy_arm_cpu(attrs, inputs, out_type, target):
    layout = attrs.data_layout
    data = inputs[0]
    strategy = _op.OpStrategy()
    interleaved_compute = topi.cuda.compute_conv2d_NHWC_quantized_interleaved_without_transform
    ...
    if layout == "NHWC" and data.dtype in ["int8", "uint8", "float32"]:
        from .arm_cpu import wrap_compute_conv2d_gemm
        strategy.add_implementation(
            wrap_compute_conv2d_gemm(interleaved_compute),
            wrap_topi_schedule(
                topi.cuda.schedule_conv2d_NHWC_quantized_interleaved_without_transform
            ),
            name="conv2d_NHWC_quantized_interleaved_without_transform.cuda",
        )
    else:
        raise RuntimeError(
...
def batch_matmul_strategy_cuda(attrs, inputs, out_type, target):
    ...
    #import sys
    #print("tvm.relay.op.strategy.cuda.batch_matmul_strategy_cuda", file=sys.stderr)
