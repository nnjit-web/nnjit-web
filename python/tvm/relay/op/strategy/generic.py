...
def conv2d_strategy(attrs, inputs, out_type, target):
    ...
    #import sys
    #print("tvm.relay.op.strategy.generic.conv2d_strategy", file=sys.stderr)
...
# conv2d_gemm
@override_native_generic_func("conv2d_gemm_strategy")
def conv2d_gemm_strategy(attrs, inputs, out_type, target):
    """conv2d_gemm generic strategy"""
    raise ValueError("No generic implemenation for conv2d_gemm")
...
def conv2d_gemm_without_weight_transform_strategy(attrs, inputs, out_type, target):
    ...
    logger.warning("conv2d_gemm_without_weight_transform is not optimized for this platform.")
    ...
    strategy.add_implementation(
        ...
        wrap_topi_schedule(topi.generic.schedule_conv2d_gemm_without_weight_transform),
        name="conv2d_gemm_without_weight_transform.generic",
    )
    return strategy
...
'''
# add
def wrap_compute_add(topi_compute):
    """wrap add topi compute"""

    def _compute_add(attrs, inputs, out_type):
        return topi_compute(*inputs)
...
def add_strategy(attrs, inputs, out_type, target):
    ...
    logger.warning("add is not optimized for this platform.")
    strategy = _op.OpStrategy()
    strategy.add_implementation(
        wrap_compute_add(topi.nn.add),
