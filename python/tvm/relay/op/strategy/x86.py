...
def softmax_strategy_cpu(attrs, inputs, out_type, target):
    ...
    strategy.add_implementation(
        wrap_compute_softmax(topi.x86.softmax),
...
def conv2d_strategy_cpu(attrs, inputs, out_type, target):
    """conv2d x86 strategy"""
    #import sys
    #print("tvm.relay.op.strategy.x86.conv2d_strategy_cpu", file=sys.stderr)
    strategy = _op.OpStrategy()
    data, kernel = inputs
    ...
    if groups == 1:
        if layout == "NCHW":
            ...
            if "dnnl" in target.libs:
                ...
                '''
                strategy.add_implementation(
                    wrap_compute_conv2d_gemm(topi.x86.compute_conv2d_NHWC_native),
                    wrap_topi_schedule(topi.x86.schedule_conv2d_NHWC_native),
                    name="conv2d_NHWC_native.x86",
                )
...
@conv2d_gemm_strategy.register("cpu")
def conv2d_gemm_strategy_cpu(attrs, inputs, out_type, target):
    """conv2d_gemm cpu strategy"""
    #import sys
    #print("tvm.relay.op.strategy.x86.conv2d_gemm_strategy_cpu", file=sys.stderr)
    layout = attrs.data_layout
    data = inputs[0]
    strategy = _op.OpStrategy()

    native_compute = topi.x86.compute_conv2d_NHWC_native
    interleaved_compute = topi.x86.compute_conv2d_NHWC_interleaved
    if layout == "NHWC" and data.dtype in ["int8", "uint8", "float32"]:
        '''
        strategy.add_implementation(
            wrap_compute_conv2d_gemm(native_compute),
            wrap_topi_schedule(
                topi.x86.schedule_conv2d_NHWC_native
            ),
            name="conv2d_NHWC_native.x86",
        )
        '''
        strategy.add_implementation(
            ...
            wrap_topi_schedule(
                topi.x86.schedule_conv2d_NHWC_interleaved
            ),
            name="conv2d_NHWC_interleaved.x86",
        )
    else:
        raise RuntimeError(
...
def wrap_compute_conv2d_gemm_without_transform(topi_compute):
    """wrap topi compute for conv2d_gemm_without_transform"""
...
@conv2d_gemm_without_weight_transform_strategy.register("cpu")
def conv2d_gemm_without_weight_transform_strategy_cpu(attrs, inputs, out_type, target):
    """conv2d_gemm_without_weight_transfrom cpu strategy"""
    #import sys
    #print("tvm.relay.op.strategy.x86.conv2d_gemm_without_weight_transform_strategy_cpu", file=sys.stderr)
    layout = attrs.data_layout
    data = inputs[0]
    strategy = _op.OpStrategy()

    native_compute = topi.x86.compute_conv2d_NHWC_native_without_transform
    interleaved_compute = topi.x86.compute_conv2d_NHWC_interleaved_without_transform
    if layout == "NHWC" and data.dtype in ["int8", "uint8", "float32"]:
        strategy.add_implementation(
            wrap_compute_conv2d_gemm_without_transform(native_compute),
            wrap_topi_schedule(
                topi.x86.schedule_conv2d_NHWC_native_without_transform
            ),
            name="conv2d_NHWC_native_without_transform.x86",
        )
        strategy.add_implementation(
            wrap_compute_conv2d_gemm_without_transform(interleaved_compute),
            wrap_topi_schedule(
                topi.x86.schedule_conv2d_NHWC_interleaved_without_transform
            ),
            name="conv2d_NHWC_interleaved_without_transform.x86",
        )
    else:
        raise RuntimeError(
...
def dense_strategy_cpu(attrs, inputs, out_type, target):
    ...
    dtype = inputs[0].dtype
    u8s8s32 = dtype == "uint8" and inputs[1].dtype == "int8" and out_type.dtype == "int32"
    enable_dense_nopack = False
    #enable_dense_nopack = True if os.getenv("ENGINE") == "tvm" else False
    enable_dense_pack = True
    if enable_dense_nopack:
        strategy.add_implementation(
...
def batch_matmul_strategy_cpu(attrs, inputs, out_type, target):
    ...
    #import sys
    #print("tvm.relay.op.strategy.x86.batch_matmul_strategy_cpu", file=sys.stderr)
    strategy = _op.OpStrategy()
    ...
    if (
        ...
        '''
        strategy.add_implementation(
            wrap_compute_batch_matmul(topi.x86.batch_matmul_interleaved, need_out_dtype=True),
            wrap_topi_schedule(topi.x86.schedule_batch_matmul_interleaved),
            name="batch_matmul_interleaved.x86",
            plevel=10,
        )
        '''
    if "cblas" in target.libs:
...
'''
@add_strategy.register(["cpu"])
def add_strategy_cpu(attrs, inputs, out_type, target):
    """add x86 strategy"""
    strategy = _op.OpStrategy()
    strategy.add_implementation(
        wrap_compute_add(topi.x86.add),
        wrap_topi_schedule(topi.x86.schedule_add),
        name="add.cpu",
    )
    return strategy
'''
