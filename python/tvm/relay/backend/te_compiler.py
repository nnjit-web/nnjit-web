...
def select_implementation(op, attrs, inputs, out_type, target, use_autotvm=True):
    ...
    """
    #print("te_compiler.py: Select impl for %s" % (op.name))
    ...
    #print("te_compiler.py: Using %s for %s based on highest priority (%s)" % (
    #        best_plevel_impl.name, op.name, best_plevel_impl.plevel))
