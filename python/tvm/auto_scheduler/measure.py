...
from tvm.contrib import ndk, tar, emcc
...
class LocalBuilder(ProgramBuilder):
    ...
    def __init__(self, timeout=15, n_parallel=multiprocessing.cpu_count(), build_func="default"):
        if build_func == "default":
            ...
            BuildFunc.name = "emscripten"
            BuildFunc.build_func = emcc.create_tvmjs_wasm
    ...
    if error_no == 0:
        ...
        try:
            ...
            with transform.PassContext().current():
                if build_func.output_format == "wasm":
            ...
            tend = time.time()
            t_build_module = tend - tstart
            tstart = time.time()
            func.export_library(filename, build_func)
            tend = time.time()
            t_export_library = tend - tstart
            #print("measure.py: filename %s, t_build_module %f, t_export_library %f" % (
            #        filename, t_build_module, t_export_library))
        # pylint: disable=broad-except
        except Exception:
...
def _timed_eval_wasm_func(
    ...
    try:
        wasm_path = build_res.filename
        wasm_binary = open(wasm_path, "rb").read()
        from tvm import rpc
        proxy_host = "127.0.0.1"
        proxy_port = os.getenv("PROXY_PORT")
        proxy_port = 9090 if proxy_port is None else proxy_port
        key = os.getenv("PROXY_KEY")
        key = "wasm" if key is None else key
        session_timeout = 10
        #print("measure.py: RPC connect")
        remote = rpc.connect(
            proxy_host,
            proxy_port,
            key=key,
        ...
        #print("measure.py: inp.task.target %s, dev %s" % (str(inp.task.target), dev))
        #print("measure.py: args.len %d, build_res.args.len %d" % (len(args), len(build_res.args)))
        ##for idx in range(len(args)):
        #    print("measure.py: idx {}, arg {}, build_res_arg_shape {}, build_res_arg_dtype {}".format(
        #        idx, args[idx], build_res.args[idx].shape, build_res.args[idx].dtype))
        ...
        for idx in range(len(args)):
            if args[idx] is None:
                build_res_arg = build_res.args[idx]
                #print("measure.py: create empty ndarray".format(
                #        build_res_arg))
                empty_array = ndarray.empty(
                    get_const_tuple(build_res_arg.shape), build_res_arg.dtype, dev
                )
                ...
                #print("measure.py: create not-empty ndarray")
                loc_args.append(ndarray.array(args[idx], dev))
        dev.sync()
        #print("measure.py: ready to run kernel")

        fname = "default_function"
        is_webgpu = "webgpu" in str(inp.task.target.keys)
        #print("measure.py: is_webgpu %s" % str(is_webgpu))
        if is_webgpu:
            ...
            ftimeexec(fname, dev, number, *loc_args)
    ...
    if verbose >= 1:
        if error_no == MeasureErrorNo.NO_ERROR:
            ...
            print("*E", end="", flush=True)  # Run error
    #print("measure.py: filename %s, t_time_evaluation %f" % (build_res.filename, (toc - tic)))
    return costs, error_no, error_msg, toc - tic + build_res.time_cost, toc
