...
from ..utils import traverse_inline, get_const_tuple, get_max_power2_factor, get_os_env_var_bool
...
def schedule_batch_matmul(cfg, outs):
    ...
    def _default_schedule_no_shared_memory(cfg, op):
        C = op.output(0)
        A, B = s[C].op.input_tensors
        if len(B.op.input_tensors) == 1 and B.op.input_tensors[0] == A:
            s[B].compute_inline()
    ...
    def _my_schedule(cfg, op):
        ...
        #cfg.define_split("tile_x", x, num_outputs=3)
        #cfg.define_split("tile_k", k, num_outputs=2)
        cfg.define_knob("tile_y", [8])
        cfg.define_knob("tile_x", [8])
        cfg.define_knob("tile_k", [8])
        cfg.define_knob("auto_unroll_max_step", [8])
        target = tvm.target.Target.current()
        if target.kind.name in ["nvptx", "rocm"]:
            # llvm-based backends cannot do non-explicit unrolling
            cfg.define_knob("unroll_explicit", [1])
        ...
        if cfg.is_fallback:
            cfg["tile_x"] = OtherOptionEntity(8)
            cfg["tile_y"] = OtherOptionEntity(8)
            cfg["tile_k"] = OtherOptionEntity(8)
            cfg["auto_unroll_max_step"] = OtherOptionEntity(16)

        ty, yi = s[C].split(y, cfg["tile_y"].val)
        by, ty = s[C].split(ty, cfg["tile_y"].val)
        tx, xi = s[C].split(x, cfg["tile_x"].val)
        bx, tx = s[C].split(tx, cfg["tile_x"].val)
        ...
        s[BL].compute_at(s[CC], ki)
        _, y, k = s[AA].op.axis
        ty, yi = s[AA].split(y, nparts=cfg["tile_y"].val)
        tx, ki = s[AA].split(k, nparts=cfg["tile_x"].val)
        s[AA].reorder(ty, tx, yi, ki)
        s[AA].bind(ty, thread_y)
        s[AA].bind(tx, thread_x)
        s[AA].pragma(yi, "auto_unroll_max_step", cfg["auto_unroll_max_step"].val)
        ...
        _, x, k = s[BB].op.axis
        ty, xi = s[BB].split(x, nparts=cfg["tile_y"].val)
        tx, ki = s[BB].split(k, nparts=cfg["tile_x"].val)
        s[BB].bind(ty, thread_y)
        s[BB].bind(tx, thread_x)
        s[BB].reorder(ty, tx, xi, ki)
        s[BB].pragma(xi, "auto_unroll_max_step", cfg["auto_unroll_max_step"].val)
    ...
    def _callback(op):
        if "batch_matmul" in op.tag:
            use_shared_memory = get_os_env_var_bool("TVM_CUDA_USE_SHARED_MEMORY", True)
            if use_shared_memory:
                _default_schedule(cfg, op)
            else:
                _default_schedule_no_shared_memory(cfg, op)
