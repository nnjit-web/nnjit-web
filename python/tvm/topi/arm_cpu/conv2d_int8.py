...
def _compute_conv2d_NHWC_quantized(
    ...
    have_kernel_transform = False
    if have_kernel_transform:
        kernel = nn.conv2d_gemm_weight_transform(kernel, tile_rows_B, tile_cols_B)
    else:
        print("conv2d_int8.py: disable kernel transformation (i.e., weight packing)")
        kernel = te.placeholder(((KH * KW * IC) // tile_rows_B, OC // tile_cols_B, tile_rows_B, tile_cols_B), name="weight_transformed")
...
def _schedule_conv2d_NHWC_quantized(cfg, outs, interleave_A):
    ...
    out = outs[0]
    '''
    if out.op.name == "conv2d_gemm_output" and not use_auto_scheduler:
        n, h, w, c = out.op.axis
    ...
    def _callback(op):
        ...
        if op.name == "conv2d_gemm_output" or op.name == "C_interleaved":
            conv_out = op.output(0)
            if interleave_A:
                schedule_conv2d_gemm_interleaved(cfg, s, conv_out, out)
            else:
                schedule_conv2d_gemm_native(cfg, s, conv_out, out)
            if op.name != "conv2d_gemm_output" or use_auto_scheduler:
                return
            return
            if out != conv_out:
...
def compute_conv2d_NHWC_quantized_interleaved_without_transform(
    ...
    #import sys
    #print("tvm.topi.arm_cpu.conv2d_int8.compute_conv2d_NHWC_quantized_interleaved_without_transform", file=sys.stderr)
...
def compute_conv2d_NHWC_quantized_native_without_transform(
    ...
    #import sys
    #print("tvm.topi.arm_cpu.conv2d_int8.compute_conv2d_NHWC_quantized_native_without_transform", file=sys.stderr)
