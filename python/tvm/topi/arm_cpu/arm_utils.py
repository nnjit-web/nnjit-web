...
def get_arch_version(target_mattr):
    ...
    for attr in target_mattr:
        ...
        #print("attr %s, match_obj %s" % (str(attr), str(match_obj)))
        if match_obj:
...
def is_dotprod_available():
    ...
    #print("target %s, target.mattr %s, arch_version %s" % (str(target), str(target.mattr), str(arch_version)))
...
def is_wasm_arm():
    target = tvm.target.Target.current(allow_none=False)
    return "wasm" in target.attrs.get("mtriple", "")
