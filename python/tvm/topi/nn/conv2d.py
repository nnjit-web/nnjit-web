...
def conv2d(
    ...
    """
    #import sys
    #print("tvm.topi.nn.conv2d", file=sys.stderr)
...
def conv2d_nchw(Input, Filter, stride, padding, dilation, out_dtype=None):
    ...
    """
    #import sys
    #print("topi.nn.conv2d_nchw", file=sys.stderr)
...
def conv2d_gemm_weight_transform(cfg, kernel, tile_rows, tile_cols):
    ...
    N_padded = N + pad_N
    K_padded = K + pad_K
    #print("conv2d.py: K_padded %d, N_padded %d" % (K_padded, N_padded))
    ...
    if conv2d_packing is None or conv2d_packing == "none":
        ...
        return te.compute(
            (N_padded // packw_bn, K_padded, packw_bn),
            lambda x, y, z: kernel_flat[y, z + packw_bn * x],
        ...
        if pre_pack:
            ...
            if transpose:
                ...
                return te.compute(
                    (K_padded // tile_rows, N_padded // tile_cols, tile_rows, tile_cols),
                    lambda x, y, z, w: kernel_flat[z + tile_rows * x, w + tile_cols * y],
                    name="weight_block_reshape",
                )
        else:
            return kernel_flat
