...
def _schedule_softmax(softmax_op, s, outs):
    ...
    def _schedule(output_op, softmax_op):
        ...
        #co, ci = s[output_op].split(output_op.axis[-1], 4)
        #s[output_op].unroll(co)
        #s[output_op].vectorize(ci)
...
@autotvm.register_topi_compute("add.x86")
def add(
    cfg, tensor_a, tensor_b
):
    return nn.add(tensor_a, tensor_b)


@autotvm.register_topi_schedule("add.x86")
def schedule_add(cfg, outs):
    s = te.create_schedule([x.op for x in outs])
    return s
