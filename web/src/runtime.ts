...
class GraphExecutorDebug extends GraphExecutor {
  private packedRunIndividual: PackedFunc;
  private packedProfileRpc: PackedFunc;
  ...
  runIndividual(
    number: number=10,
    repeat: number=1,
    minRepeatMs: number=0,
    limitZeroTimeIterations: number=0,
    cooldownIntervalMs: number=0,
    repeatsToCooldown: number=0
  ): Uint8Array {
    return this.packedRunIndividual(
      new Scalar(number, "int32"),
      new Scalar(repeat, "int32"),
      new Scalar(minRepeatMs, "int32"),
      new Scalar(limitZeroTimeIterations, "int32"),
      new Scalar(cooldownIntervalMs, "int32"),
      new Scalar(repeatsToCooldown, "int32")
    );
  }
...
export class Instance implements Disposable {
  ...
  private graphExecutor: GraphExecutor | undefined;
  private graphExecutorDebug: GraphExecutorDebug | undefined;
  ...
  initWebGPU(device: GPUDevice): void {
    ...
    this.registerFunc("wasm.WebGPUDeviceAPI", (name: string): Function => {
      return webGPUContext.getDeviceAPI(name);
    });
    this.registerFunc("wasm.WebGPURegisterShader", (name: string) => {
      this.registerFunc(name, this.systemLib().getFunction(name));
    });
    this.registerFunc("wasm.WebGPUCreateShader", (name: string, info: string, data: Uint8Array): Function => {
  ...
  private registerEnvGlobalPackedFuncs(): void {
    ...
    const runGraphExecutorForWasm = (...args: Array<any>) => {
      const dev = args[0];
      const repeat = args[1];
      const result = [];
      if (this.graphExecutor != undefined) {
    ...
    const runGraphExecutorForWebGPU = (...args: Array<any>) => {
      // Register shader functions.
      const shaderNames = this.lib.webGPUContext?.getShaderNames();
    ...
    const runIndividualGraphExecutorDebug = (...args: Array<any>): void => {
      if (this.graphExecutorDebug != undefined) {
        ...
        this.graphExecutorDebug.runIndividual(
          nstep, repeat, minRepeatMs,
          limitZeroTimeIterations, cooldownIntervalMs, repeatsToCooldown
        );
      }
    };

    const profileGraphExecutorDebug = (): string => {
      if (this.graphExecutorDebug != undefined) {
