...
class WebGPUDeviceAPI : public DeviceAPI {
 ...
 public:
  ...
  void* AllocWorkspace(Device dev, size_t size, DLDataType type_hint) final {
    void *p = WebGPUThreadEntry::ThreadLocal()->pool.AllocWorkspace(dev, size);
    LOG(INFO) << "AllocWorkspace: size " << size << ", ptr " << reinterpret_cast<int>(p);
    return p;
  }

  void FreeWorkspace(Device dev, void* data) final {
    LOG(INFO) << "FreeWorkspace: ptr " << reinterpret_cast<int>(data);
...
class WebGPUModuleNode final : public runtime::ModuleNode {
 public:
  explicit WebGPUModuleNode(std::unordered_map<std::string, VulkanShader> smap,
    ...
    CHECK(fp != nullptr);
    create_shader_ = *fp;
    auto* rsfp = tvm::runtime::Registry::Get("wasm.WebGPURegisterShader");
    CHECK(rsfp != nullptr);
    register_shader_ = *rsfp;
    // NOTE(fucheng): Registering shader here will cause error.
    //for (auto& it : fmap) {
    //  std::string name = it.first;
    //  register_shader_(name);
    //}
#if 0
    LOG(INFO) << "WebGPUModuleNode is created";
#endif
  }
  ...
  PackedFunc GetFunction(const std::string& name, const ObjectPtr<Object>& sptr_to_self) final {
    ...
    if (it != smap_.end()) {
      ...
      PackedFunc pf = create_shader_(name, os.str(), arr);
      if (pf == nullptr) {
        LOG(WARNING) << "Null function " << name;
      }
#if 0
      LOG(INFO) << "Register function to global: " << name;
      TVMFuncRegisterGlobal(name.c_str(), &pf, 1);
#endif
      return pf;
    } else {
#if 1
      LOG(WARNING) << "Can not find function " << name;
#endif
 ...
 private:
  ...
  TypedPackedFunc<PackedFunc(std::string fname, std::string finfo, TVMByteArray shader_data)> create_shader_;
  // Callback to register the GPU shader function.
  TypedPackedFunc<void(std::string fname)> register_shader_;
};
...
Module WebGPUModuleLoadBinary(void* strm) {
  ...
  for (auto& it : fmap) {
    std::string name = it.first;
    VulkanShader shader = smap.at(name);
    FunctionInfo info = fmap.at(name);
    size_t source_size = shader.data.size() * sizeof(shader.data[0]);
#if 0
    LOG(INFO) << "name " << name
              << ", func_info_name " << info.name
 ...
 public:
  ...
  PackedFunc GetFunction(const std::string& name, const ObjectPtr<Object>& sptr_to_self) final {
    ...
    if (it != smap_.end()) {
      ...
      info.Save(&writer);
      TVMByteArray arr;
      arr.data = const_cast<char*>(it->second.c_str());
      arr.size = it->second.size();
      PackedFunc pf = create_shader_(name, os.str(), arr);
      if (pf == nullptr) {
        LOG(WARNING) << "Null function " << name;
      }
